	...
	$('.block').each(function(period) {
		...
		while (i<=classes.length - 1) {
			...
			if (classes[i][3] == period) {
				$(this).append('<span class="class-selectors" type="checkbox" name="' + classes[i] + '" title="Meets ' + classes[i][4].toString().replace(/false/gi, "").replace(/,/gi, "").toUpperCase() + ' (in ' + classes[i][5] +')">' + classes[i][1] + ', ' + classes[i][2] + '</span><br />');
